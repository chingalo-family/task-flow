# Task Flow Documentation

Welcome to the **Task Flow** documentation! This directory contains comprehensive guides and references for understanding, using, and contributing to the Task Flow application.

## 📚 Documentation Index

### Getting Started
- [**Installation & Setup**](./GETTING_STARTED.md) - Step-by-step guide to set up and run the application
- [**Quick Start Guide**](./QUICK_START.md) - Get up and running quickly
- [**macOS Setup**](./MACOS_SETUP.md) - macOS-specific setup and troubleshooting

### Core Documentation
- [**Overview**](./OVERVIEW.md) - Introduction to Task Flow, its purpose, and key benefits
- [**Features**](./FEATURES.md) - Comprehensive list of all application features and capabilities
- [**Architecture**](./ARCHITECTURE.md) - Technical architecture and project structure
- [**Technology Stack**](./TECH_STACK.md) - Technologies, frameworks, and libraries used

### User Guides
- [**User Guide**](./USER_GUIDE.md) - Complete guide for end users
- [**Task Management**](./TASK_MANAGEMENT.md) - How to manage tasks effectively
- [**Team Collaboration**](./TEAM_COLLABORATION.md) - Working with teams

### Developer Documentation
- [**API & Services**](./API_SERVICES.md) - Backend services and API documentation
- [**State Management**](./STATE_MANAGEMENT.md) - Application state architecture
- [**Reusable Components**](./REUSABLE_COMPONENTS.md) - UI component library
- [**ObjectBox Setup**](./OBJECTBOX_BUILD_REQUIRED.md) - Database setup and configuration
- [**Contributing**](./CONTRIBUTING.md) - Guidelines for contributing to the project

### Additional Resources
- [**Roadmap**](./ROADMAP.md) - Upcoming features and future plans
- [**FAQ**](./FAQ.md) - Frequently asked questions
- [**Changelog**](./CHANGELOG.md) - Version history and changes

## 🚀 Quick Links

### For Users
- New to Task Flow? Start with the [Overview](./OVERVIEW.md)
- Ready to use? Check the [User Guide](./USER_GUIDE.md)

### For Developers
- Setting up? See [Installation & Setup](./GETTING_STARTED.md)
- Contributing? Read the [Contributing Guide](./CONTRIBUTING.md)
- Understanding the code? Review the [Architecture](./ARCHITECTURE.md)

## 📖 About This Documentation

This documentation is organized to help different audiences:
- **End Users**: Learn how to use Task Flow effectively
- **Developers**: Understand the codebase and contribute
- **Administrators**: Set up and maintain the application
- **Contributors**: Follow best practices and guidelines

## 🆘 Need Help?

- Check the [FAQ](./FAQ.md) for common questions
- Review the [User Guide](./USER_GUIDE.md) for detailed instructions
- See [Getting Started](./GETTING_STARTED.md) for setup issues

## 🌐 Related Resources

- **Website**: [https://chingalo-family.github.io/task-flow-website/](https://chingalo-family.github.io/task-flow-website/)
- **Website Source Code**: [https://github.com/chingalo-family/task-flow-website](https://github.com/chingalo-family/task-flow-website)
- **API**: [https://vmi2503861.contaboserver.net/task-flow-api/](https://vmi2503861.contaboserver.net/task-flow-api/)
- **API Documentation**: [https://vmi2503861.contaboserver.net/task-flow-api-docs/](https://vmi2503861.contaboserver.net/task-flow-api-docs/)
- **API Source Code**: [https://github.com/chingalo-family/task-flow-api](https://github.com/chingalo-family/task-flow-api)
- **Design Resources**: [https://stitch.withgoogle.com/projects/14437076708101911838](https://stitch.withgoogle.com/projects/14437076708101911838)

## 📝 Contributing to Documentation

Found an error or want to improve the docs? See our [Contributing Guide](./CONTRIBUTING.md) for information on how to help improve this documentation.
